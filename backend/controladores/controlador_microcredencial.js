...
const obtenerDetalleMicrocredencialPublica = async (req, res) => {
  try {
    const { id } = req.params;
    const consulta = `
      SELECT 
      ...
      JOIN area_conocimiento a ON m.area_conocimiento = a.id_area
      JOIN estado_microcredencial e ON m.estado = e.id_estado
      WHERE m.id_microcredencial = $1 AND m.eliminado = false AND m.estado = 2
    `;
    const result = await consultar(consulta, [id]);
    if (result.rows.length === 0) {
      return res.status(404).json({ exito: false, mensaje: 'Microcredencial no encontrada o no disponible.' });
    }
    return res.status(200).json({
      exito: true,
      datos: result.rows[0]
    ...
    console.error('Error al obtener detalle de microcredencial pública:', error.message);
    return res.status(500).json({ exito: false, mensaje: 'Error al obtener la microcredencial.' });
  }
};
...
module.exports = {
  ...
  obtenerDetalleMicrocredencialPublica
};
