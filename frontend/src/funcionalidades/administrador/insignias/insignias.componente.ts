...
export class InsigniasAdminComponente implements OnInit, OnDestroy {
      ...
      for (const sortKey of activeSorts) {
        ...
        switch (sortKey) {
          ...
          case 'microcredencial':
            ...
            break;
          case 'duracion':
            const durA = parseInt((a.duracion || '0').replace(/\D/g, '')) || 0;
            const durB = parseInt((b.duracion || '0').replace(/\D/g, '')) || 0;
            cmp = durA - durB;
            break;
          case 'estado':
  ...
  cargarInsigniasEmitidas() {
    this.subscription.add(
      this.insigniaServicio.obtenerHistorialGeneral().subscribe({
        next: (res: any) => {
          if (res && res.exito) {
